...
const ProjectCard = ({ imgSrc, techs, currTechs, Title, description, gitLink, liveLink }) => {
    ...
    return (
        ...
        >
            <div
                className={`p-5 sm:p-8 font-poppins  bg-[#000319] transition-all border-[1px] shadow-2xl shadow-cyan-950 border-slate-800 flex flex-col items-start justify-center gap-5 rounded-2xl`}
            >
                <div className="rounded-3xl max-h-72 pt-5 overflow-y-hidden overflow-x-hidden bg-gradient-to-r backdrop-blur-lg from-gray-900 via-cyan-950 to-slate-900 p-3 sm:p-4">
                    <img
                        className={`rounded-xl rotate-3 transition-all shadow-xl shadow-cyan-950`}
                        src={imgSrc}
                        alt="ProjectImage"
                    />
                </div>
